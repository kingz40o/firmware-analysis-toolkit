...
cd binwalk
sed -i '/git clone --quiet --depth 1 --branch "master" https:\/\/github.com\/jrspruitt\/ubi_reader/c\$SUDO $PYTHON -mpip install --user ubi_reader' deps.sh
sed -i 's/$SUDO rm -rf ubi_reader//g' deps.sh
sed -i 's/(cd ubi_reader && $SUDO $PYTHON setup.py install)//g' deps.sh
